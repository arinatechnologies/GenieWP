 ...
 */
function geniewp_create_initial_content() {
	// Check if already setup
	if ( get_option( 'geniewp_theme_setup_complete' ) ) {
		return;
	}
	...
	$pages = array(
		'Home' => array(
			'content' => geniewp_get_home_content( $theme_data ),
			'template' => 'front-page'
		),
		'About' => array(
			'content' => geniewp_get_about_content( $theme_data ),
			'template' => ''
		),
		'Services' => array(
			'content' => geniewp_get_services_content( $theme_data ),
			'template' => ''
		),
		'Blog' => array(
			'content' => geniewp_get_blog_content( $theme_data ),
			'template' => ''
		),
		'Contact' => array(
			'content' => geniewp_get_contact_content( $theme_data ),
			'template' => ''
		),
	);
	...
	// Mark as complete
	update_option( 'geniewp_theme_setup_complete', true );
}
add_action( 'after_switch_theme', 'geniewp_create_initial_content' );

/**
 * Get home page content with hero
 */
function geniewp_get_home_content( $theme_data ) {
...
<h1 class="has-text-align-center has-white-color has-text-color">' . esc_html( $theme_data['content']['hero']['headline'] ) . '</h1>
<!-- /wp:heading -->

<!-- wp:paragraph {"align":"center","textColor":"white","fontSize":"large"} -->
<p class="has-text-align-center has-white-color has-text-color has-large-font-size">' . esc_html( $theme_data['content']['hero']['subheadline'] ) . '</p>
<!-- /wp:paragraph -->

<!-- wp:buttons {"layout":{"type":"flex","justifyContent":"center"}} -->
<div class="wp-block-buttons">
<!-- wp:button {"backgroundColor":"primary","textColor":"white","className":"is-style-fill"} -->
<div class="wp-block-button is-style-fill"><a class="wp-block-button__link has-white-color has-primary-background-color has-text-color has-background wp-element-button">' . esc_html( $theme_data['content']['hero']['cta_text'] ) . '</a></div>
<!-- /wp:button -->
</div>
<!-- /wp:buttons -->
</div></div>
 ...
 */
function geniewp_get_about_content( $theme_data ) {
...
<!-- wp:heading -->
<h2>About ' . esc_html( $theme_data['site_name'] ) . '</h2>
<!-- /wp:heading -->

<!-- wp:paragraph -->
<p>' . esc_html( $theme_data['content']['about']['content'] ) . '</p>
<!-- /wp:paragraph -->
...
<!-- wp:paragraph -->
<p>We are dedicated to providing exceptional ' . strtolower( $theme_data['business_type'] ) . ' services that exceed your expectations.</p>
<!-- /wp:paragraph -->
 ...
 */
function geniewp_get_services_content( $theme_data ) {
...
<p class="has-text-align-center has-large-font-size">We offer a comprehensive range of ' . strtolower( $theme_data['business_type'] ) . ' services to meet your needs.</p>
<!-- /wp:paragraph -->
...
<div class="wp-block-columns">';
	...
	$i = 0;
	foreach ( $theme_data['content']['services'] as $service ) {
		$services_html .= '
	<!-- wp:column -->
	<div class="wp-block-column">
		...
		<h3 class="has-text-align-center">' . esc_html( $service['title'] ) . '</h3>
		<!-- /wp:heading -->
		
		<!-- wp:paragraph {"align":"center"} -->
		<p class="has-text-align-center">' . esc_html( $service['description'] ) . '</p>
		<!-- /wp:paragraph -->
	</div>
	<!-- /wp:column -->';
		$i++;
 ...
 */
function geniewp_get_blog_content( $theme_data ) {
 ...
 */
function geniewp_get_contact_content( $theme_data ) {
...
<div class="wp-block-columns">
	...
	<div class="wp-block-column">
		...
		<div class="wp-block-group has-light-gray-background-color has-background" style="border-radius:8px;padding-top:var(--wp--preset--spacing--50);padding-right:var(--wp--preset--spacing--50);padding-bottom:var(--wp--preset--spacing--50);padding-left:var(--wp--preset--spacing--50)">
			...
			<!-- wp:paragraph {"align":"center"} -->
			<p class="has-text-align-center">info@' . sanitize_title( $theme_data['site_name'] ) . '.com</p>
			<!-- /wp:paragraph -->
		</div>
		<!-- /wp:group -->
	</div>
